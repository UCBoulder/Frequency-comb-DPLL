...
class ML_sampler:
    ...
    def create_sparse_sampling(self):
        # get PID ranges
        (_, _, _, _, _, fmin, fmax, gain_min, gain_max,
         _) = self.qloop_filters.getSettings()

        # Added my own lower bounds from experience
        gain_bounds = np.array([-3e2, gain_max])
        offset = gain_bounds[0]
        gain_bounds -= offset
        coeff_bounds = np.array([0, fmax])
        gain_bounds += 1
        coeff_bounds += 1
        # Convert to log scale for reasonable sampling
        gain_bounds = np.log10(gain_bounds)
...
#        print('coeff bounds: %s' % str(coeff_bounds_slider_units))
        ...
        # int(round(np.log(samples)/np.log(5),0)) # number of steps in each dimension to sparsely cover the space
        spd = 3

        # Steps in which to somewhat evenly sample the space
        gain_step = (gain_bounds[1] - gain_bounds[0]) / (spd + 1)
                ...
                for k in range(1, spd + 1):
                    for l in range(1, spd + 1):
                        for m in range(1, spd + 1):
                            gain_val = np.power(
                                gain_bounds[0] + gain_step * i, 10) + offset
                            fi_val = np.power(
                                coeff_bounds[0] + coeff_step * j, 10)
                            fii_val = np.power(
                                coeff_bounds[0] + coeff_step * k, 10)
                            fd_val = np.power(
                                coeff_bounds[0] + coeff_step * l, 10)
                            fdf_val = np.power(
                                coeff_bounds[0] + coeff_step * m, 10)
                            # Last empty value is for appending the outcome
                            sampling.append(
                                [gain_val, fi_val, fii_val, fd_val, fdf_val, 0])

        # put into numpy array format for easy indexing!
    ...
    def create_localized_sampling(self):
        ...
        if self.qloop_filters != None:
            ...
            (kp, fi, fii, fd, fdf, fmin, fmax, gain_min,
             gain_max, bLock) = self.qloop_filters.getSettings()
        else:
            kp = 1
            fi = 1
            fii = 1
        ...
        # Creates random state vectors. Will need to be updated for higher dimensions
        rands = np.random.rand(25, 2)
        # Use a lognormal distribution to evenly cover vals above and below setpoint
        lognormal_rands = np.random.lognormal(0, 1.5, 50)
...
#        print(sampling[:,-1]) # These will be the output values after sampling
    ...
    def update_training_data(self, completed_sampling):
        # Just the first two variables for now
        training_data = completed_sampling[:, :2]
        outputs = completed_sampling[:, -1]        # Median Integrated PSD

        self.gp.fit(training_data, outputs)

    # Use current gp to minimize the integrated PSD and set PIDs to that value
    def optimize_lock(self):
        (kp, fi, fii, fd, fdf, fmin, fmax, gain_min, gain_max,
         # Get settings and use to set bounds
         bLock) = self.qloop_filters.getSettings()
        bounds = [(0.1*kp, 10*kp), (0.1*fi, 10*fi)]
        # Creates 26 random points in parameter space
        sampling = self.create_localized_sampling()
        best_setting = [sampling[0], 100]
        for params in sampling:
            guess = params[:2]  # First two values for now
            # , maxiter = 1e3) # This is a dictionary object
            xopt = optimize.minimize(
                self.gp_prediction, guess, method='L-BFGS-B', tol=1e-5, bounds=bounds)
            # If the gp predict a better lock, update the recommended settings
            if xopt['fun'] < best_setting[1]:
