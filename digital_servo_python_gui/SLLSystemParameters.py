...
#        self.Writejson('hi_jshaw', "") # settings remain empty
    ...
    def populateDefaults(self):
        ...
        self.root.append(ET.Element('Reference_frequency',
                         DDC0='31.25e6', DDC1='31.25e6'))
        self.root.append(ET.Element(
            'VCO gain', DAC0='2e8', DAC1='0.5e6', DAC2='9e6'))
        self.root.append(ET.Element('Output_limits_low',
                         DAC0='-1.0', DAC1='-0', DAC2='0'))
        self.root.append(ET.Element('Output_limits_high',
                         DAC0='1.0', DAC1='1', DAC2='55'))
        self.root.append(ET.Element('Input_Output_gain',
                         ADC0='1', ADC1='1', DAC0='1', DAC1='1'))
        self.root.append(ET.Element('Output_offset_in_volts',
                         DAC0='0.0274', DAC1='0', DAC2='27'))
        self.root.append(ET.Element('PLL0_settings', kp='10', fi='45e3', fii='3.4e3', fd='1',
                         fdf='1', chkKd='False', chkKp='False', chkLock='False', chkKpCrossing='False'))
        self.root.append(ET.Element('PLL1_settings', kp='-5.6', fi='141e3', fii='3.24e3',
                         fd='1', fdf='1', chkKd='False', chkKp='True', chkLock='False', chkKpCrossing='True'))
        self.root.append(ET.Element('PLL2_settings', kp='-120', fi='1e-2', fii='0', fd='1',
                         fdf='1', chkKd='False', chkKp='False', chkLock='False', chkKpCrossing='False'))

        self.root.append(ET.Element('PWM0_settings', standard='3.3',
                         levels='256', default='0.0', minval='0.0', maxval='3.3'))

        self.root.append(ET.Element('Main_window_settings', refresh_delay='500',
                         N_samples_adc='1.75e3', N_samples_ddc='1e6', Integration_limit='5e6'))
        self.root.append(ET.Element(
            'Triangular_averaging', DAC1='1', DAC0='1'))

        self.root.append(ET.Element(
            'Dither_frequency', DAC1='5.1e3', DAC0='1e3'))
        self.root.append(ET.Element(
            'Dither_integration_time', DAC1='0.1', DAC0='0.1'))
        self.root.append(ET.Element(
            'Dither_amplitude', DAC1='1e.3', DAC0='1e-3'))
        self.root.append(ET.Element('Dither_mode', DAC1='2', DAC0='2'))

        self.root.append(ET.Element(
            'VCO_settings', VCO_offset='0.00', VCO_amplitude='0.5', VCO_connection='0'))
        self.root.append(ET.Element(
            'RP_settings', Fan_state='0', PLL1_connection='0'))
...
#        self.tree = ET.fromstring(strFilename, parser=parser)
    ...
    def sendToFPGA(self, bSendToFPGA=True):
        ...
        # the limit is in volts
        limit_low = float(self.getValue('Output_limits_low', 'DAC0'))
        # the limit is in volts
        limit_high = float(self.getValue('Output_limits_high', 'DAC0'))
        self.sl.set_dac_limits(0,
                               int(limit_low/self.sl.dev.DAC_V_INT),
                               int(limit_high/self.sl.dev.DAC_V_INT))
        # the limit is in volts
        limit_low = float(self.getValue('Output_limits_low', 'DAC1'))
        # the limit is in volts
        limit_high = float(self.getValue('Output_limits_high', 'DAC1'))
        self.sl.set_dac_limits(1,
                               int(limit_low/self.sl.dev.DAC_V_INT),
                               int(limit_high/self.sl.dev.DAC_V_INT))
        # the limit is in volts
        limit_low = float(self.getValue('Output_limits_low', 'DAC2'))
        # the limit is in volts
        limit_high = float(self.getValue('Output_limits_high', 'DAC2'))
        ...
        value_in_counts = self.sl.convertPWMVoltsToCounts(
            PWM0_standard, PWM0_levels, PWM0_default)
        # Send to FPGA
