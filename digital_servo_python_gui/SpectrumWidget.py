    ...
    def initUI(self):
        ...
        for k in range(3):
            if self.output_controls[k] == True:
                ...
                self.qthermo_dac_current[k].setRange(self.sl.convertDACCountsToVolts(
                    k, self.sl.DACs_limit_low[k]), self.sl.convertDACCountsToVolts(k, self.sl.DACs_limit_high[k]))
                self.qthermo_dac_current[k].setScale(self.sl.convertDACCountsToVolts(
                    k, self.sl.DACs_limit_low[k]), self.sl.convertDACCountsToVolts(k, self.sl.DACs_limit_high[k]))
...
#        print(self.qplt_IQ.sizeHint())
        ...
        self.curve_IQ = self.qplt_IQ.getPlotItem().plot(
            pen=None, symbol='o', symbolPen=None, symbolSize=3, symbolBrush='b')
        ...
        for k in range(3):
            if self.output_controls[k] == True:
                ...
                grid.addWidget(
                    self.qthermo_dac_current[k],    1, 2+N_dac_controls, 3, 1)
                grid.addWidget(
                    self.qlabel_dac_current_value[k], 4, 2+N_dac_controls, 1, 1)
                grid.addWidget(
                    self.qlabel_dac_offset[k],      0, 3+N_dac_controls)
                grid.addWidget(
                    self.q_dac_offset[k],           1, 3+N_dac_controls, 3, 1)
                grid.addWidget(
                    self.qlabel_dac_offset_value[k], 4, 3+N_dac_controls, 1, 1)
...
#        grid.addItem(spacerItem, 9, 0, 1, 2)
    ...
    def plotADCdata(self, input_select, plot_type, samples_out, ref_exp0):
        ...
        complex_baseband = self.sl.frontend_DDC_processing(
            samples_out, ref_exp0, self.selected_ADC)
    ...
    def handleComplexBaseband(self, complex_baseband, plot_type):
        ...
        if self.bDisplayTiming == True:
            print('Elapsed time (complex baseband plots) = %f' %
                  (time.perf_counter()-start_time))
        start_time = time.perf_counter()
    ...
    def plotADCorDACspectrum(self, samples_out, input_select):
        ...
        # Scale from the modulus square of the FFT to the (double-sided) power spectra
        spc = np.real(spc * np.conj(spc))/(np.sum(window_function)**2)
        spc_single_sided_psd = spc*2/self.computeNEB(window_function, self.sl.fs) * (
            2**15*self.sl.convertADCCountsToVolts(self.selected_ADC, 1))**2
        # Measure average PSD level by looking at out-of-band noise and rejecting outliers:
        def index_from_freq(freq): return round(
            freq*N_fft/self.sl.fs)  # f_axis = index/N_fft*fs
        ind_min_psd = index_from_freq(10e6)
        ind_max_psd = index_from_freq(20e6)
        # slice out an out-of-band section
        spc_single_sided_psd = spc_single_sided_psd[ind_min_psd:ind_max_psd]
        ...
        spc_single_sided_psd = np.delete(
            spc_single_sided_psd, worst_outlier_index)
        ...
        self.plt_spc.setTitle('Spectrum, noise floor = %.0f nV/sqrt(Hz)' %
                              (round_to_N_sig_figs(1e9*np.sqrt(avg_psd), 2)))
    ...
    def plotADCorDACtimeDomain(self, samples_out, input_select):
        ...
        # NEB doesn't make that much sense here, but we still plot 1/Total time
        self.updateNEBdisplay(self.sl.fs/len(samples_out))

        time_axis = np.linspace(0, len(samples_out)-1,
                                len(samples_out))/self.sl.fs
        ...
        self.plt_spc.setYRange((valmin+valmax)/2-1.1*(valmax-valmin) /
                               2-1/65e3, (valmin+valmax)/2+1.1*(valmax-valmin)/2+1/65e3)
        self.plt_spc.setXRange(time_axis[0], time_axis[-1])

        self.plt_spc.setTitle(
            'Time-domain signal, std = %.2f mV' % (1e3*np.std(samples_out)))
        self.plt_spc.setTitle('Time, std = %.2f mV, ampl = %.2f mVpp' % (
            1e3*np.std(samples_out), 1e3*np.std(samples_out)*2*np.sqrt(2)))
    ...
    def updateFilterSpcDisplay(self, frequency_axis):
        ...
        if self.bDisplayTiming == True:
            print('Elapsed time (Spectrum of filter) = %f' %
                  (time.perf_counter()-start_time))
    ...
    def updateIQdisplay(self, complex_baseband, N_max_IQ=10e3):
        ...
        complex_baseband = complex_baseband[:int(
            np.min((len(complex_baseband), N_max_IQ)))]
    ...
    def plotPhaseData(self, complex_baseband):
        ...
        time_axis = np.linspace(
            0, len(complex_baseband)-1, len(complex_baseband))/self.sl.fs
        ...
        self.plt_spc.setTitle(
            'Time-domain phase, std = %.2f radrms' % np.std(phi))
    ...
    def plotIQData(self, complex_baseband, bPhaseAligned):
        ...
        if bPhaseAligned:
            ...
            complex_baseband = complex_baseband * \
                np.conj(complex_baseband[0])/np.abs(complex_baseband[0])
    ...
    def setDACOffset_event(self, value=0):
        for k in range(3):
            if self.output_controls[k] == True:
                ...
                counts_offset = int(self.sl.DACs_limit_low[k] + float(self.sl.DACs_limit_high[k] - self.sl.DACs_limit_low[k]) * float(
                    # counts_offset is 16 bits signed
                    self.q_dac_offset[k].value())/1e6)
                self.sl.set_dac_offset(k, counts_offset)

                VCO_gain_in_Hz_per_Volts = self.parent.getVCOGainFromUI(k)

                # Update the display:
                current_output_in_volts = self.sl.convertDACCountsToVolts(
                    k, counts_offset)
    ...
    def getDACoffset(self):
        for k in range(3):
            if self.output_controls[k] == True:
                ...
                q_dac_offset = float(counts_offset-self.sl.DACs_limit_low[k])*1e6/float(
                    self.sl.DACs_limit_high[k] - self.sl.DACs_limit_low[k])
    ...
    def setSliderStepSize(self, output_number, VCO_gain_in_Hz_per_Volts):
        ...
        if output_number == 1 and self.sl.read_pll2_mux() == 2:
            ...
            # use hard-coded value because the textbox is used for a different setting (gain ratio instead of VCO gain)
            VCO_gain_in_Hz_per_Volts = 700e6
            small_step = int(round(1e6/3. * (0.5e6 / float(VCO_gain_in_Hz_per_Volts) / float(self.sl.getDACGainInVoltsPerCounts(
                output_number))) / float(self.sl.DACs_limit_high[output_number] - self.sl.DACs_limit_low[output_number])))
            large_step = int(round(1e6 * (5e6 / float(VCO_gain_in_Hz_per_Volts) / float(self.sl.getDACGainInVoltsPerCounts(
                output_number))) / float(self.sl.DACs_limit_high[output_number] - self.sl.DACs_limit_low[output_number])))
        else:
            # Normal operation
            small_step = int(round(1e6/3. * (0.5e6 / float(VCO_gain_in_Hz_per_Volts) / float(self.sl.getDACGainInVoltsPerCounts(
